...
#elif defined(ESP8266) || defined(ESP32)
#define SPI_DEFAULT_FREQ 60000000  // Increased from 40MHz for better performance
#elif defined(RASPI)
    ...
    {          // each font "column" (+1 blank column for spacing)
        ...
        { // each column byte
            ...
            for (uint8_t k = 0; k < 8; k++)
            {
                ...
                if (fastMode)
                    _writeData16(bitRead(charData, k) ? color : _bgColor);
                else
                    drawPixel(x + i, y + (j * 8) + k, bitRead(charData, k) ? color : _bgColor);
                h++;
            }
        }
    }
